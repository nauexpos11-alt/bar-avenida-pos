...
function contarItems(cuenta) {
  return cuenta?.productosCount ?? cuenta?.ordenesCount ?? 0
}
      ...
      <div className="ccm-modal">
        ...
        <div className="ccm-header">
          <span className="ccm-header-title">
            COBRAR CUENTA — Mesa {mesaNum} — Folio #{folio}
            {cuenta?.meseraNombre ? ` — Mesera: ${cuenta.meseraNombre}` : ''}
          </span>
          {fase === 'form' && (
              ...
              <div className="ccm-fase-card ccm-fase-ok">
                ...
                {resultado?.modoSimulado && (
                  <div className="ccm-fase-sim">
                    📄 Ticket simulado generado — revísalo en Configuración
                  </div>
                )}
              </div>
            )}
